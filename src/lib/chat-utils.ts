...
// Cache to prevent excessive parsing of the same response
const parseCache = new Map<string, FlightData[]>();

export function parseFlightDataFromResponse(response: string): FlightData[] {
  // Check cache first to prevent excessive parsing
  if (parseCache.has(response)) {
    return parseCache.get(response)!;
  }
  
  // Only log if response contains potential flight data
  if (response.includes('ICAO24') || response.includes('icao24') || response.includes('|')) {
    console.log('Parsing flight data from response:', response.substring(0, 100) + '...');
  }
  ...
  // Only log when actually finding codes to reduce console noise
  if (icao24Codes.length > 0) {
  ...
  // Cache the result to prevent re-parsing
  parseCache.set(response, flights);
